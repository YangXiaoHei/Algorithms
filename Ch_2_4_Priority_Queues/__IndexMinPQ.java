...
public class __IndexMinPQ <Key extends Comparable<Key>> implements Iterable<Key> {
    private int[] pq;
    private int[] qp;
    private Key[] keys;
    private int size;
    @SuppressWarnings("unchecked")
    public __IndexMinPQ(int N) {
        pq = new int[N + 1];
        qp = new int[N + 1];
    ...
    public Iterator<Key> iterator() {
        return new Iterator<Key>() {
            private __IndexMinPQ<Key> copy = new __IndexMinPQ<Key>(keys.length);
            {
    ...
    public static void main(String[] args) {
        ...
        __IndexMinPQ<String> pq = new __IndexMinPQ<String>(s.length);
        for (String ss : s)
            pq.insert(ss);
        for (String i : pq) 
            StdOut.println(i);
